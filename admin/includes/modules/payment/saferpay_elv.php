 ...
 */
    ...
    function osC_Payment_saferpay_elv() {
      ...
      $this->_description = $osC_Language->get('payment_saferpay_elv_description');
      $this->_method_title = $osC_Language->get('payment_saferpay_elv_method_title');
      $this->_status = (defined('MODULE_PAYMENT_SAFERPAY_ELV_STATUS') && (MODULE_PAYMENT_SAFERPAY_ELV_STATUS == '1') ? true : false);
 ...
 */
    ...
    function install() {
      ...
      $osC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Enable Saferpay Bank Transfer Module', 'MODULE_PAYMENT_SAFERPAY_ELV_STATUS', '-1', 'Do you want to accept Saferpay bank transfer payments?', '6', '0', 'osc_cfg_use_get_boolean_value', 'osc_cfg_set_boolean_value(array(1, -1))', now())");
      $osC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Account ID', 'MODULE_PAYMENT_SAFERPAY_ELV_ACCOUNT_ID', '', 'The account ID of the Saferpay account to use.', '6', '0', now())");
      $osC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Special Hosting Password', 'MODULE_PAYMENT_SAFERPAY_ELV_PASSWORD', '', 'The special hosting password to use when connecting to the payment gateway.', '6', '0', now())");
      $osC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Payment Zone', 'MODULE_PAYMENT_SAFERPAY_ELV_ZONE', '0', 'If a zone is selected, only enable this payment method for that zone.', '6', '2', 'osc_cfg_use_get_zone_class_title', 'osc_cfg_set_zone_classes_pull_down_menu', now())");
      $osC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Set Order Status', 'MODULE_PAYMENT_SAFERPAY_ELV_ORDER_STATUS_ID', '0', 'Set the status of orders made with this payment module to this value', '6', '0', 'osc_cfg_set_order_statuses_pull_down_menu', 'osc_cfg_use_get_order_status_title', now())");
    ...
    function getKeys() {
      if (!isset($this->_keys)) {
        $this->_keys = array('MODULE_PAYMENT_SAFERPAY_ELV_STATUS',
                             'MODULE_PAYMENT_SAFERPAY_ELV_ACCOUNT_ID',
                             'MODULE_PAYMENT_SAFERPAY_ELV_PASSWORD',
                             'MODULE_PAYMENT_SAFERPAY_ELV_ZONE',
                             'MODULE_PAYMENT_SAFERPAY_ELV_ORDER_STATUS_ID',
                             'MODULE_PAYMENT_SAFERPAY_ELV_SORT_ORDER');
      }
