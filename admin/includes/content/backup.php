...
// check if the backup directory exists
      if (is_dir(DIR_FS_BACKUP)) {
        if (!is_writeable(DIR_FS_BACKUP)) {
          $osC_MessageStack->add($this->_module, ERROR_BACKUP_DIRECTORY_NOT_WRITEABLE, 'error');
        }
      } else {
        $osC_MessageStack->add($this->_module, ERROR_BACKUP_DIRECTORY_DOES_NOT_EXIST, 'error');
      }
...
/* Private methods */
    ...
    function _forget() {
      ...
      $osC_MessageStack->add_session($this->_module, SUCCESS_LAST_RESTORE_CLEARED, 'success');
...
// dump the data
      ...
      if (isset($_POST['download']) && ($_POST['download'] == 'yes')) {
        ...
        $osC_MessageStack->add_session($this->_module, SUCCESS_DATABASE_SAVED, 'success');
      }

      osc_redirect(osc_href_link_admin(FILENAME_DEFAULT, $this->_module));
    }

    function _restore($local = false) {
      global $osC_Database, $osC_MessageStack;
...
// join the query before the comment appeared, with the rest of the dump
        ...
        $osC_MessageStack->add_session($this->_module, SUCCESS_DATABASE_RESTORED, 'success');
      }
      ...
      if (isset($_GET['file'])) {
        if (file_exists(DIR_FS_BACKUP . basename($_GET['file']))) {
          if (osc_remove(DIR_FS_BACKUP . basename($_GET['file']))) {
            $osC_MessageStack->add_session($this->_module, SUCCESS_BACKUP_DELETED, 'success');
