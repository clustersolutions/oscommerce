...
// check if the catalog image directory exists
      ...
      if (!empty($_GET['action'])) {
        switch ($_GET['action']) {
          case 'new':
            $this->_page_contents = 'products_edit.php';
          ...
          case 'preview':
            $this->_page_contents = 'products_preview.php';
            break;
...
/* Private methods */
    ...
    function _move() {
      global $osC_Database, $osC_MessageStack, $current_category_id;
      ...
      if (isset($_GET['pID']) && is_numeric($_GET['pID'])) {
        ...
        if ($Qcheck->valueInt('total') < 1) {
          ...
          if ($Qupdate->affectedRows()) {
            ...
            osc_redirect(osc_href_link_admin(FILENAME_DEFAULT, $this->_module . '&page=' . $_GET['page'] . '&cPath=' . $_POST['move_to_category_id'] . '&search=' . $_GET['search'] . '&pID=' . $_GET['pID']));
          } else {
      ...
      osc_redirect(osc_href_link_admin(FILENAME_DEFAULT, 'page=' . $_GET['page'] . '&cPath=' . $_GET['cPath'] . '&search=' . $_GET['search'] . '&pID=' . $_GET['pID']));
    }

    function _save() {
      global $osC_Database, $osC_Language, $osC_MessageStack, $osC_Image;
      ...
      osc_redirect(osc_href_link_admin(FILENAME_DEFAULT, $this->_module . '&page=' . $_GET['page'] . '&cPath=' . $_GET['cPath'] . '&search=' . $_GET['search'] . '&pID=' . $products_id));
    }
    ...
    function _copy() {
        ...
        if ($_POST['copy_as'] == 'link') {
          if (end($category_array) != $current_category_id) {
            ...
            if ($Qcheck->valueInt('total') < 1) {
              ...
              if ($Qcat->affectedRows()) {
                ...
                osc_redirect(osc_href_link_admin(FILENAME_DEFAULT, $this->_module . '&page=' . $_GET['page'] . '&cPath=' . $_POST['categories_id'] . '&search=' . $_GET['search'] . '&pID=' . $_GET['pID']));
              }
            }
          } else {
            $osC_MessageStack->add_session($this->_module, ERROR_CANNOT_LINK_TO_SAME_CATEGORY, 'error');
          }
          ...
          if ($Qproduct->numberOfRows() === 1) {
            ...
            if ($error === false) {
              ...
              osc_redirect(osc_href_link_admin(FILENAME_DEFAULT, $this->_module . '&page=' . $_GET['page'] . '&cPath=' . $_POST['categories_id'] . '&search=' . $_GET['search'] . '&pID=' . $new_product_id));
            } else {
              $osC_Database->rollbackTransaction();
      ...
      osc_redirect(osc_href_link_admin(FILENAME_DEFAULT, $this->_module . '&page=' . $_GET['page'] . '&cPath=' . $_GET['cPath'] . '&search=' . $_GET['search'] . '&pID=' . $_GET['pID']));
    }
  }
?>
