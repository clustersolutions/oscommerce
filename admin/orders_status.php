  ...
  if (!empty($action)) {
    switch ($action) {
      case 'save':
        ...
        osc_redirect(osc_href_link_admin(FILENAME_ORDERS_STATUS, 'page=' . $_GET['page'] . '&osID=' . $orders_status_id));
        break;
      case 'deleteconfirm':
        if (isset($_GET['osID']) && is_numeric($_GET['osID'])) {
          ...
          if ( (DEFAULT_ORDERS_STATUS_ID == $_GET['osID']) || ($Qorders->valueInt('total') > 0) || ($Qhistory->valueInt('total') > 0) ) {
            ...
            osc_redirect(osc_href_link_admin(FILENAME_ORDERS_STATUS, 'page=' . $_GET['page'] . '&osID=' . $_GET['osID']));
          } else {
            ...
            osc_redirect(osc_href_link_admin(FILENAME_ORDERS_STATUS, 'page=' . $_GET['page']));
          }
        }
        break;
    }
