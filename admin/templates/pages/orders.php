...
<?php
  echo '<form name="orders" action="' . osc_href_link_admin(FILENAME_ORDERS) . '" method="get">' .
...
<?php
  ...
  while ($Qorders->next()) {
    ...
    if (isset($oInfo) && ($Qorders->valueInt('orders_id') == $oInfo->orders_id)) {
      ...
      echo '      <tr onmouseover="rowOverEffect(this);" onmouseout="rowOutEffect(this);" onclick="document.location.href=\'' . osc_href_link_admin(FILENAME_ORDERS, (isset($_GET['search']) ? 'search=' . $_GET['search'] . '&' : '') . (isset($_GET['status']) ? 'status=' . $_GET['status'] . '&' : '') . (isset($_GET['cID']) ? 'cID=' . $_GET['cID'] . '&' : '') . 'page=' . $_GET['page'] . '&oID=' . $Qorders->valueInt('orders_id')) . '\';">' . "\n";
    }
?>
        ...
        <td align="right">
<?php
    echo '<a href="#" onclick="document.location.href=\'' . osc_href_link_admin(FILENAME_ORDERS, (isset($_GET['search']) ? 'search=' . $_GET['search'] . '&' : '') . (isset($_GET['status']) ? 'status=' . $_GET['status'] . '&' : '') . (isset($_GET['cID']) ? 'cID=' . $_GET['cID'] . '&' : '') . 'page=' . $_GET['page'] . '&oID=' . $Qorders->valueInt('orders_id') . '&action=oEdit') . '\';">' . osc_icon('configure.png', IMAGE_EDIT) . '</a>&nbsp;';

    if (isset($oInfo) && ($Qorders->valueInt('orders_id') == $oInfo->orders_id)) {
      echo '<a href="#" onclick="toggleInfoBox(\'oDelete\');">' . osc_icon('trash.png', IMAGE_DELETE) . '</a>';
    } else {
      echo osc_link_object(osc_href_link_admin(FILENAME_ORDERS, (isset($_GET['search']) ? 'search=' . $_GET['search'] . '&' : '') . (isset($_GET['status']) ? 'status=' . $_GET['status'] . '&' : '') . (isset($_GET['cID']) ? 'cID=' . $_GET['cID'] . '&' : '') . 'page=' . $_GET['page'] . '&oID=' . $Qorders->valueInt('orders_id') . '&action=oDelete'), osc_icon('trash.png', IMAGE_DELETE));
    }
?>
        </td>
      </tr>
...
?>
  <p align="right"><?php echo '<input type="button" value="' . IMAGE_RESET . '" class="operationButton" onclick="document.location.href=\'' . osc_href_link_admin(FILENAME_ORDERS) . '\';"> '; ?></p>
<?php
  }
?>
...
<div id="infoBox_oDelete" <?php if ($action != 'oDelete') { echo 'style="display: none;"'; } ?>>
  ...
  <div class="infoBoxContent">
    <form name="oDelete" action="<?php echo osc_href_link_admin(FILENAME_ORDERS, (isset($_GET['search']) ? 'search=' . $_GET['search'] . '&' : '') . (isset($_GET['status']) ? 'status=' . $_GET['status'] . '&' : '') . (isset($_GET['cID']) ? 'cID=' . $_GET['cID'] . '&' : '') . 'page=' . $_GET['page'] . '&oID=' . $oInfo->orders_id . '&action=deleteconfirm'); ?>" method="post">
