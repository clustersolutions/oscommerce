...
<div id="infoBox_wcDefault" <?php if (!empty($_GET['action'])) { echo 'style="display: none;"'; } ?>>
...
<?php
    if (SHIPPING_WEIGHT_UNIT == $Qclasses->valueInt('weight_class_id')) {
      echo '<b>' . $Qclasses->value('weight_class_title') . ' (' . TEXT_DEFAULT . ')</b>';
    } else {
      echo $Qclasses->value('weight_class_title');
    }
?>
...
<?php
    if (isset($wcInfo) && ($Qclasses->valueInt('weight_class_id') == $wcInfo->weight_class_id)) {
      echo osc_link_object('#', osc_icon('configure.png', IMAGE_EDIT), 'onclick="toggleInfoBox(\'wcEdit\');"') . '&nbsp;' .
           osc_link_object('#', osc_icon('trash.png', IMAGE_DELETE), 'onclick="toggleInfoBox(\'wcDelete\');"');
    } else {
      echo osc_link_object(osc_href_link_admin(FILENAME_DEFAULT, $osC_Template->getModule() . '&page=' . $_GET['page'] . '&wcID=' . $Qclasses->valueInt('weight_class_id') . '&action=wcEdit'), osc_icon('configure.png', IMAGE_EDIT)) . '&nbsp;' .
           osc_link_object(osc_href_link_admin(FILENAME_DEFAULT, $osC_Template->getModule() . '&page=' . $_GET['page'] . '&wcID=' . $Qclasses->valueInt('weight_class_id') . '&action=wcDelete'), osc_icon('trash.png', IMAGE_DELETE));
    }
?>
...
<div id="infoBox_wcNew" <?php if ($_GET['action'] != 'wcNew') { echo 'style="display: none;"'; } ?>>
...
<div id="infoBox_wcEdit" <?php if ($_GET['action'] != 'wcEdit') { echo 'style="display: none;"'; } ?>>
  <div class="infoBoxHeading"><?php echo osc_icon('configure.png', IMAGE_EDIT) . ' ' . $wcInfo->weight_class_title; ?></div>
  <div class="infoBoxContent">
    <form name="wcEdit" action="<?php echo osc_href_link_admin(FILENAME_DEFAULT, $osC_Template->getModule() . '&page=' . $_GET['page'] . '&wcID=' . $wcInfo->weight_class_id . '&action=save'); ?>" method="post">
...
<div id="infoBox_wcDelete" <?php if ($_GET['action'] != 'wcDelete') { echo 'style="display: none;"'; } ?>>
...
?>
    ...
    <p align="center"><?php echo '<input type="button" value="' . IMAGE_DELETE . '" class="operationButton" onclick="document.location.href=\'' . osc_href_link_admin(FILENAME_DEFAULT, $osC_Template->getModule() . '&page=' . $_GET['page'] . '&wcID=' . $wcInfo->weight_class_id . '&action=deleteconfirm') . '\';"> <input type="button" value="' . IMAGE_CANCEL . '" onclick="toggleInfoBox(\'wcDefault\');" class="operationButton">'; ?></p>
