...
<?php
  ...
  while ($Qclasses->next()) {
    ...
    if (isset($tcInfo) && ($Qclasses->valueInt('tax_class_id') == $tcInfo->tax_class_id)) {
      ...
      echo '      <tr onmouseover="rowOverEffect(this);" onmouseout="rowOutEffect(this);" onclick="document.location.href=\'' . osc_href_link_admin(FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&tcID=' . $Qclasses->valueInt('tax_class_id')) . '\'" title="' . $Qclasses->valueProtected('tax_class_description') . '">' . "\n";
    }
?>
        <td><?php echo osc_link_object(osc_href_link_admin(FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&tcID=' . $Qclasses->valueInt('tax_class_id') . '&action=list'), osc_image('images/icons/folder.gif', ICON_FOLDER) . '&nbsp;' . $Qclasses->value('tax_class_title')); ?></td>
        <td><?php echo $Qrates->valueInt('total_tax_rates'); ?></td>
        <td align="right">
<?php
    if (isset($tcInfo) && ($Qclasses->valueInt('tax_class_id') == $tcInfo->tax_class_id)) {
      echo '<a href="#" onclick="toggleInfoBox(\'tcEdit\');">' . osc_icon('configure.png', IMAGE_EDIT) . '</a>&nbsp;' .
           '<a href="#" onclick="toggleInfoBox(\'tcDelete\');">' . osc_icon('trash.png', IMAGE_DELETE) . '</a>';
    } else {
      echo osc_link_object(osc_href_link_admin(FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&tcID=' . $Qclasses->valueInt('tax_class_id') . '&action=tcEdit'), osc_icon('configure.png', IMAGE_EDIT)) . '&nbsp;' .
           osc_link_object(osc_href_link_admin(FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&tcID=' . $Qclasses->valueInt('tax_class_id') . '&action=tcDelete'), osc_icon('trash.png', IMAGE_DELETE));
    }
?>
        </td>
      </tr>
...
<div id="infoBox_tcNew" <?php if ($action != 'tcNew') { echo 'style="display: none;"'; } ?>>
  ...
  <div class="infoBoxContent">
    <form name="tcNew" action="<?php echo osc_href_link_admin(FILENAME_TAX_CLASSES, 'action=save'); ?>" method="post">
...
<div id="infoBox_tcEdit" <?php if ($action != 'tcEdit') { echo 'style="display: none;"'; } ?>>
  ...
  <div class="infoBoxContent">
    <form name="tcEdit" action="<?php echo osc_href_link_admin(FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&tcID=' . $tcInfo->tax_class_id . '&action=save'); ?>" method="post">
