...
?>
      ...
      <tr onmouseover="rowOverEffect(this);" onmouseout="rowOutEffect(this);" title="<?php echo $Qzones->valueProtected('geo_zone_description'); ?>">
        <td><?php echo osc_link_object(osc_href_link_admin(FILENAME_DEFAULT, $osC_Template->getModule() . '&page=' . $_GET['page'] . '&zID=' . $Qzones->valueInt('geo_zone_id') . '&action=list'), osc_image('images/icons/folder.gif', ICON_FOLDER) . '&nbsp;' . $Qzones->value('geo_zone_name')); ?></td>
...
<?php
    if (isset($zInfo) && ($Qzones->valueInt('geo_zone_id') == $zInfo->geo_zone_id)) {
      ...
      echo osc_link_object(osc_href_link_admin(FILENAME_DEFAULT, $osC_Template->getModule() . '&page=' . $_GET['page'] . '&zID=' . $Qzones->valueInt('geo_zone_id') . '&action=zEdit'), osc_icon('configure.png', IMAGE_EDIT)) . '&nbsp;' .
           osc_link_object(osc_href_link_admin(FILENAME_DEFAULT, $osC_Template->getModule() . '&page=' . $_GET['page'] . '&zID=' . $Qzones->valueInt('geo_zone_id') . '&action=zDelete'), osc_icon('trash.png', IMAGE_DELETE));
    }
?>
...
<div id="infoBox_zEdit" <?php if ($_GET['action'] != 'zEdit') { echo 'style="display: none;"'; } ?>>
  <div class="infoBoxHeading"><?php echo osc_icon('new.png', IMAGE_INSERT) . ' ' . TEXT_INFO_HEADING_NEW_ZONE; ?></div>
  <div class="infoBoxContent">
    <form name="zEdit" action="<?php echo osc_href_link_admin(FILENAME_DEFAULT, $osC_Template->getModule() . '&page=' . $_GET['page'] . '&zID=' . $zInfo->geo_zone_id . '&action=save'); ?>" method="post">
