  ...
  if (!empty($action)) {
    switch ($action) {
      case 'save':
        ...
        if (isset($_GET['lID']) && is_numeric($_GET['lID'])) {
          tep_redirect(tep_href_link(FILENAME_TEMPLATES_BOXES_LAYOUT, 'set=' . $set . '&filter=' . $filter_template . '&lID=' . $_GET['lID']));
        } else {
          tep_redirect(tep_href_link(FILENAME_TEMPLATES_BOXES_LAYOUT, 'set=' . $set . '&filter=' . $filter_template));
        }
