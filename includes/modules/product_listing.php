    ...
    while ($Qlisting->next()) {
      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        ...
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_BUY_NOW':
            $lc_align = 'center';
            $lc_text = osc_link_object(OSCOM::getLink(null, null, $OSCOM_Product->getKeyword() . '&' . osc_get_all_get_params(array('action')) . '&action=cart_add'), osc_draw_image_button('button_buy_now.gif', OSCOM::getDef('button_buy_now'))) . '&nbsp;';
            break;
        }
