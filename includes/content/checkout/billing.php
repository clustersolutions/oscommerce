...
*/
  ...
  class osC_Checkout_Billing extends osC_Template {
    protected $_module = 'billing';
    protected $_group = 'checkout';
    protected $_page_title;
    protected $_page_contents = 'billing.php';
    protected $_page_image = 'table_background_payment.gif';

    public function __construct() {
      global $osC_ShoppingCart, $osC_Customer, $osC_Services, $osC_Breadcrumb, $osC_Payment, $osC_MessageStack, $osC_oiAddress;
...
// check product type perform_order conditions
      foreach ( $osC_ShoppingCart->getProducts() as $product ) {
        ...
        if ( !$osC_Product->isTypeActionAllowed('perform_order', 'require_billing') ) {
          osc_redirect(osc_href_link(FILENAME_CHECKOUT, null, 'SSL'));
        }
      }

// verify selected payment method
      if ( isset($_POST['subaction']) && ($_POST['subaction'] == 'save') ) {
        $this->_process();
      }
...
// process new address
      ...
      $this->_page_title = __('payment_method_heading');

      if ( $osC_Services->isStarted('breadcrumb') ) {
        $osC_Breadcrumb->add(__('breadcrumb_checkout_payment'), osc_href_link(FILENAME_CHECKOUT, $this->_module, 'SSL'));
      }

// load billing address page if no default address exists or billing address page is requested
      if ( isset($_GET['address']) || !$osC_ShoppingCart->hasBillingAddress() ) {
        $this->_page_title = __('payment_address_heading');
        $this->_page_contents = 'billing_address.php';

        $this->addJavascriptFilename('templates/' . $this->getCode() . '/javascript/checkout_payment_address.js');
        $this->addJavascriptPhpFilename('includes/form_check.js.php');

        if ( !$osC_Customer->isLoggedOn() ) {
          $osC_oiAddress = new osC_ObjectInfo($osC_ShoppingCart->getBillingAddress());
        }
      } else {
...
// load all enabled payment modules
      ...
      if (isset($_GET['payment_error']) && is_object(${$_GET['payment_error']}) && ($error = ${$_GET['payment_error']}->get_error())) {
        $osC_MessageStack->add('checkout_billing', $error['error'], 'error');
      }
    }
    ...
    protected function _process() {
      ...
      if ( isset($_POST['payment_method']) && !empty($_POST['payment_method']) ) {
        ...
        if ( $osC_Payment->hasActive() && isset($GLOBALS['osC_Payment_' . $_POST['payment_method']]) && ($GLOBALS['osC_Payment_' . $_POST['payment_method']] instanceof osC_Payment) && $GLOBALS['osC_Payment_' . $_POST['payment_method']]->isEnabled() ) {
          $osC_ShoppingCart->setBillingMethod(array('id' => $GLOBALS['osC_Payment_' . $_POST['payment_method']]->getCode(), 'title' => $GLOBALS['osC_Payment_' . $_POST['payment_method']]->getMethodTitle()));
      ...
      if ( !empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['street_address']) ) {
        ...
        if ( $error === false ) {
          if ( $osC_Customer->isLoggedOn() ) {
            ...
            if ( osc_empty($osC_Customer->getName()) ) {
            ...
            $osC_ShoppingCart->setBillingAddress($address_array);
            $osC_ShoppingCart->resetBillingMethod();
...
// process the selected billing destination
        ...
        if ( osC_AddressBook::checkEntry($_POST['ab']) ) {
          $osC_ShoppingCart->setBillingAddress($_POST['ab']);
          $osC_ShoppingCart->resetBillingMethod();
