...
*/
  ...
  namespace osCommerce\OM\Core\Site\Shop\Application\Checkout\Action\Shipping\Address;
        ...
        if ( $error === false ) {
          if ( $OSCOM_Customer->isLoggedOn() ) {
            ...
            if ( $Qab->rowCount() === 1 ) {
              ...
              $OSCOM_ShoppingCart->setShippingAddress($address_book_id);
...
// process the selected shipping destination
        ...
        if ( AddressBook::checkEntry($_POST['ab']) ) {
          ...
          OSCOM::redirect(OSCOM::getLink(null, null, 'Shipping', 'SSL'));
        } else {
          OSCOM::redirect(OSCOM::getLink(null, null, 'Shipping&Address', 'SSL'));
        }
      } else {
        OSCOM::redirect(OSCOM::getLink(null, null, 'Confirm', 'SSL'));
      }
