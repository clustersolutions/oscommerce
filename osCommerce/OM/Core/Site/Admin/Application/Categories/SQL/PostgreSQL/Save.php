 ...
 */
  ...
  namespace osCommerce\OM\Core\Site\Admin\Application\Categories\SQL\PostgreSQL;
  ...
  class Save {
    public static function execute($data) {
      ...
      if ( is_numeric($data['id']) ) {
        ...
        $Qcat = $OSCOM_PDO->prepare('insert into :table_categories (parent_id, date_added) values (:parent_id, now()) returning categories_id');
      }
      ...
      if ( !$Qcat->isError() ) {
        $category_id = ( is_numeric($data['id']) ) ? $data['id'] : $Qcat->valueInt('categories_id');
