  ...
  class Language extends \osCommerce\OM\Core\Site\Admin\Language {
    ...
    public function loadIniFile($filename = null, $comment = '#', $language_code = null) {
      ...
      if ( is_null($filename) ) {
        ...
        if ( substr(realpath(OSCOM::BASE_DIRECTORY . 'Site/' . OSCOM::getSite() . '/languages/' . $language_code . '/' . $filename), 0, strlen(realpath(OSCOM::BASE_DIRECTORY . 'Site/' . OSCOM::getSite() . '/languages/' . $language_code))) != realpath(OSCOM::BASE_DIRECTORY . 'Site/' . OSCOM::getSite() . '/languages/' . $language_code) ) {
          return array();
        }
