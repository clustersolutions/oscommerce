        ...
        try {
            ...
            ob_start(function($buffer) {
                foreach (headers_list() as $h) {
                    if (stripos($h, 'Location:') !== false) {
                        ...
                        \osCommerce\OM\Core\HttpRequest::setResponseCode(403); // ob_start() needs full namespace

                        $buffer = json_encode([
                            'rpcStatus' => constant('osCommerce\\OM\\Core\\Site\\RPC\\Controller::STATUS_REDIRECT_DETECTED')
                        ]);
            ...
            for ($i = $run_pos, $n = count($_GET); $i < $n; $i++) {
                ...
                if (static::siteApplicationRPCExists(implode('\\', $rpc))) {
                    call_user_func(['osCommerce\\OM\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . OSCOM::getSiteApplication() . '\\RPC\\' . implode('\\', $rpc), 'execute']);
