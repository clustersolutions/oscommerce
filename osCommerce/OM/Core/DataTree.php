...
class DataTree
{
    ...
    public function getChildren(int $id, bool $recursive = false): array
    {
        $result = [];

        if (isset($this->data[$id])) {
            foreach ($this->data[$id] as $category_id => $category) {
                $r = array_merge([
                    'id' => $category_id
                ], $category);

                if (($recursive === true) && isset($this->data[$category_id])) {
                    $r['children'] = $this->getChildren($category_id, true);
                }
    ...
    {
        foreach ($this->data as $pid => $pinfo) {
            foreach ($pinfo as $cid => $cinfo) {
                if ($cid === $id) {
                    return $pid;
                }
            }
    ...
    public function getFullPath(int $id, string $key = null): array
    {
        $result = [];

        if ($id > 0) {
            $result[] = $this->getData($id, $key);

            while (($id = $this->getParentId($id)) !== 0) {
                $result[] = $this->getData($id, $key);
            }
    ...
    public function getData(int $id, string $key = null)
    {
        $data = $this->data[$this->getParentId($id)][$id];
 ...
 */
    ...
    public function getData2($id, $key = null) {
      foreach ( $this->_data as $parent => $categories ) {
        foreach ( $categories as $category_id => $info ) {
          if ( $id == $category_id ) {
            $data = array('id' => $id,
 ...
 */
    ...
    public function getParentID2($id) {
      return $this->getData2($id, 'parent_id');
    }
